...
# ---- H2O (local training only) ----
import h2o
from h2o.automl import H2OAutoML

# ---- sklearn export (for Streamlit) ----
...
def parse_args():
    import argparse
    p = argparse.ArgumentParser(description="Train DGA detector with H2O AutoML (MOJO) and sklearn (pickle)")
...
def main():
    ...
    # -------------------- sklearn model (for Streamlit Cloud) --------------------
    y_bin = (Xy[args.label_col].values == "DGA").astype(int)
    X_train, X_val, y_train, y_val = train_test_split(X.values, y_bin, test_size=0.2, random_state=args.seed, stratify=y_bin)
    clf = XGBClassifier(
        n_estimators=300,
        max_depth=4,
        learning_rate=0.1,
    ...
    skl_path = out_dir / "DGA_Leader_sklearn.pkl"
    joblib.dump({"model": clf, "feature_order": x_cols}, skl_path)
    print(f"Exported sklearn model -> {skl_path}")
